...
- [x] 9.1. Создать `.golangci.yml`:
  - Включить linters: `errcheck`, `gosimple`, `go vet`, `goimports`, `ineffassign`, `staticcheck`, `typecheck`, `unused`
  - Настроить `run.timeout = 5m`
- [x] 9.2. Добавить обработку ошибок повсеместно (без тихих провалов)
- [x] 9.3. Добавить timeout контекста в HTTP-хендлеры
- [x] 9.4. Добавить README.md с:
  - Описанием проекта
  - Эндпоинтами API
  - Как запустить (`make run`)
  - Как протестировать (`make test`)
- [x] 9.5. Запустить `golangci-lint run ./...` → 0 warnings
- [x] 9.6. Убедиться, что все тесты проходят с `go test -race -count 100 ./...`
- [x] 9.7. Убедиться, что `internal/bandit`, `internal/service`, `internal/model` не импортируют сторонние пакеты
